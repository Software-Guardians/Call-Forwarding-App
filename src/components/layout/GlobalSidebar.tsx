...
export const GlobalSidebar: React.FC<GlobalSidebarProps> = ({ currentView, onNavigate }) => {
    ...
    const getLinkClass = (viewName: string) => {
        ...
        return "group flex items-center gap-4 px-3 py-3 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 transition-all w-full text-left";
    };

    const getIconClass = (viewName: string) => {
        const isActive = currentView === viewName;
        return isActive ? "material-symbols-outlined" : "material-symbols-outlined group-hover:text-primary transition-colors";
    };
    ...
    return (
        <aside className="w-20 lg:w-64 h-full flex flex-col border-r border-gray-200 dark:border-white/5 bg-white dark:bg-[#080c10] z-20 transition-all duration-300 shrink-0">
            {/* Logo Area */}
            <div className="h-24 flex items-center gap-3 px-6 border-b border-gray-200 dark:border-white/5">
            ...
            <nav className="flex-1 flex flex-col gap-2 p-4 overflow-y-auto">
                ...
                <button className="group flex items-center gap-4 px-3 py-3 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 transition-all w-full text-left">
